...
$(function() {
    ...
    $('#category-table').on('click', '.btn-edit', function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        $.get('/categories/edit/' + id, function(response) {
            $('#edit-category-id').val(response.id);
            $('#edit-name').val(response.name);
            $('#edit-active').val(response.active);

            if (response.photo) {
                $('#current-image').html('<img src="{{ asset('uploads/category') }}/' + response.photo + '" width="80" style="border-radius:4px;">');
            } else {
            ...
            // Remove any old method field
            $('#editCategoryForm').find('input[name="_method"]').remove();
            $('#editCategoryForm').prepend('<input type="hidden" name="_method" value="PUT">');
