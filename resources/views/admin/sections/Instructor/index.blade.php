    ...
    <div class="container-fluid">
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    <button class="btn btn-sm btn-secondary" type="button"
                                                        id="actionDropdown{{ $data->id }}" data-bs-toggle="dropdown"
                                                        aria-expanded="false">
                                                        <i class="fas fa-ellipsis-v"></i> {{-- 3 vertical dots --}}
                                                    </button>
                                                    <ul class="dropdown-menu"
                                                        ...
                                                        <li>
                                                            <form
                                                                action="{{ route('web_pages.Instructor.destroy', $data->id) }}"
                                                                method="POST" class="delete-instructor-form"
                                                                data-id="{{ $data->id }}">
                                                                @csrf
                                                                <button type="submit"
        ...
        <div class="modal fade" id="addProfessionalModal" tabindex="-1" aria-labelledby="addProfessionalModalLabel"
            ...
            <div class="modal-dialog">
                <form method="POST" action="{{ route('web_pages.Instructor.store') }}">
                    @csrf
                ...
                <form method="POST" id="editProfessionalForm">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label>Button Text</label>
                                <input type="text" name="button" id="edit_button" class="form-control">
                            </div>
...
@push('script')
    <script>
        $('.edit-btn').on('click', function() {
            ...
            $.ajax({
                url: "/website_frontend/Instructor/edit/" + id,
                method: "GET",
                success: function(res) {
                    if (res.status && res.data) {
                        ...
                        $('#edit_button').val(data.button);
                        $('#edit_button_url').val(data.button_url);

                        $('#editProfessionalForm').attr('action',
                            '/website_frontend/Instructor/update/' + data
                            .id);
                    }
                },
                error: function() {
                    alert('Failed to fetch instructor details.');
                }
            });
        });

        $(document).on('submit', '.delete-instructor-form', function(e) {
